...
func update_slots():
    ...
    for slot_index in range(0, max_slots):
        if get_child_count() - 1 < slot_index:
            ...
            var grid_item = preload("res://Scenes/InventoryGridSlot.tscn").instance()
        ...
        if (item_count > slot_index):
            ...
            grid_item.texture = load("res://Assets/icon.png")
        else:
            grid_item.texture = load("res://Assets/slot.png")
        
func _input(event):
    if event.is_action("ui_give_item") and event.is_pressed():
